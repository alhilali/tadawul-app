...
export class FirebaseAuthService {
  ...
  constructor(public angularFire: AngularFireAuth, public platform: Platform, @Inject(PLATFORM_ID) private platformId: object) {
        ...
        this.angularFire.getRedirectResult().then(
          (result) => {
            ...
            if (result.user) {
              ...
              // Work around, since subject lisnters are not listening to this message yet
              setTimeout(() => {
                this.redirectResult.next(result);
              }, 3000);
            }
  ...
  private setUserModelForProfile(): FirebaseProfileModel {
    ...
    const providerData = this.currentUser.providerData[0];
    const userData = this.userProviderAdditionalInfo ? this.userProviderAdditionalInfo : providerData;
    userModel.image = this.getPhotoURL(providerData.providerId, providerData.photoURL);
    userModel.name = userData.name || userData.displayName;
    // userModel.role = 'How would you describe yourself?';
    userModel.description = userData.description;
    ...
    userModel.token = this.currentToken;
