...
export class EventService {
  ...
  public getListingDataSource(): Observable<EventListingModel> {
    const rawDataSource = this.http.get<EventListingModel>(environment.apiBaseUrl + '/events/my').pipe(
  ...
  public getDetailsDataSource(id: string): Observable<EventDetailsModel | any> {
    const rawDataSource = this.http.get<EventDetailsModel>(environment.apiBaseUrl + '/events?q=' + id).pipe(
      map((data: any) => {
        if (!data.data || (data.data && data.data.length == 0)) {
          return { error: 'Could not find event with code ' };
        }
        ...
        // (see: https://scotch.io/bar-talk/copying-objects-in-javascript#toc-using-spread-elements-)
        Object.assign(details, data.data[0]);
  ...
  public addEvent(name: string, code: string, startDate: string, endDate: string) {
  ...
  public addQuestion(eventID: string, title: string) {
    return this.http.post<EventListingModel>(environment.apiBaseUrl + '/questions', {
      title,
      eventID,
    });
  }
